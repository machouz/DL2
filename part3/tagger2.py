...
class MLP(nn.Module):
    def __init__(self, output_layer, pre_trained_vec, embedding_dim=50, window_size=5,
                 hidden_layer=100):
...
if __name__ == '__main__':
    ...
    train_data = torch.LongTensor(
        zip(train_vecs[:, 0], train_vecs[1:, 0], train_vecs[2:, 0], train_vecs[3:, 0], train_vecs[4:, 0],
            train_vecs[2:, 1]))
    parameters = filter(lambda p: p.reqires_grad, model.parameters())
